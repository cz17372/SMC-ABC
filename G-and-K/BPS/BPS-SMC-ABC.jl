...
function α1(x0,u0,δ;y,ϵ,Dist)
    x1,u1 = φ1(x0,u0,δ)
    return min(0,logpi(x1,y=y,ϵ=ϵ,Dist=Dist) - logpi(x0,y=y,ϵ=ϵ,Dist=Dist))
end
function α2(x0,u0,δ;y,ϵ,BounceType,gradFunc,Dist)
    x2,u2 = φ2(x0,u0,δ,BounceType=BounceType,gradFunc=gradFunc)
    forward_1st_proposal_acc = exp(α1(x0,u0,δ,y=y,ϵ=ϵ,Dist=Dist))
    backward_1st_proposal_acc = exp(α1(x2,u2,δ,y=y,ϵ=ϵ,Dist=Dist))
    return min(0,log(1-backward_1st_proposal_acc)-log(1-forward_1st_proposal_acc)+logpi(x2,y=y,ϵ=ϵ,Dist=Dist)-logpi(x0,y=y,ϵ=ϵ,Dist=Dist))
end

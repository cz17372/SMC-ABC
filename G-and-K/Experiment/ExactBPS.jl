...
Information = (seed=seed,NoData=nodata,N=N,T=T,Threshold=Threshold,κ=κ,MaxBounce=MaxBounce,MaxStep=δ,MinStep=MinStepSize)

EPSILON = Array{Any,1}(undef,m)
K       = Array{Any,1}(undef,m)
α       = Array{Any,1}(undef,m)
Theta   = Array{Any,1}(undef,m)
AveBounces = Array{Any,1}(undef,m)

for i = 1:m
    R = ExactBPS.SMC(N,T,dat20,Threshold=Threshold,δ = δ,κ=κ,K0 = 5,MaxBounces=MaxBounce,MinStepsize=MinStepSize)
    EPSILON[i] = R.EPSILON
    K[i]       = R.K 
    α[i]       = R.AcceptanceProb
    Index      = findall(R.WEIGHT[:,end] .> 0)
    Theta[i]   = R.U[1:4,Index,end]
    AveBounces[i] = R.AveBounceNo
end

Results = (Information=Information, EPSILON=EPSILON,K=K,alpha=α,Theta=Theta,AveBounces=AveBounces)
